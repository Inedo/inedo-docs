...
For example, installing the [YouTrack extension](  [https://github.com/Inedo/inedox-youtrack](https://github.com/Inedo/inedox-youtrack)) will allow you to create a:
...
- Issue Source that identifies which YouTrack issues belong in which [BuildMaster release]; in YouTrack, this is done in HOW MANY WAYS (ATTN: JOHN):
	- (ATTN: JOHN) Field mapping: choosing a project in JIRA, and then picking a field in that project (like "fixFor") that BuildMaster will use to search for issues associated with the release
	- (ATTN: JOHN) Custom mapping: using JIRA's powerful JQL language to search using any number of fields (such as "fixFor", "issueType", etc.), or even across multiple JIRA projects
...
## Troubleshooting: Issue Synchronization {#troubleshooting-issue-synchronization data-title="Troubleshooting"} 
...
**Example: Inedo Use Case** [ATTN: JOHN, THIS MIGHT NOT BE RIGHT; I WAS GUESSING]
In our own public issue tracker, [ProGet's issues](https://inedo.myjetbrains.com/youtrack/issues/PG) that are associated with a specific release managed in BuildMaster are automatically changed from "Awaiting Release" to "Completed" when deployment occurs to our Release stage.
...
### Example: Basic JIRA Workflow Transition [ATTN: JOHN]
...
![](/resources/images/icons/issue-new-feature.png)  [ATTN: JOHN]
...
## Adding Issue Comments/Notes During Build/ Deploy Time {#issue-comments data-title="Adding Issue Comments During Build/Deploy Time"} 
...
### Example: Linking to "Disposable Infrastucture" Created

- Let’s say you create a unique IIS website in the integration environment for each deployment
- Testers will have no idea where to test
- Show OtterScript that will…
	- Iterate issues in using [FindYouTrackIssuesOperation.cs](https://github.com/Inedo/inedox-youtrack/blob/master/YouTrack/InedoExtension/Operations/FindYouTrackIssuesOperation.cs "FindYouTrackIssuesOperation.cs")
- Add a comment: [AddYouTrackCommentOperation.cs](https://github.com/Inedo/inedox-youtrack/blob/master/YouTrack/InedoExtension/Operations/AddYouTrackCommentOperation.cs "AddYouTrackCommentOperation.cs")
- Transition the status: [ChangeYouTrackIssueStateOperation.cs](https://github.com/Inedo/inedox-youtrack/blob/master/YouTrack/InedoExtension/Operations/ChangeYouTrackIssueStateOperation.cs "ChangeYouTrackIssueStateOperation.cs")
...
foreach $IssueId in @YouTrackIssueIDs
{
	...
	(
		Credentials: CorpYouTrack,
		IssueId: $IssueId,
		Comment: Automatically updated by BuildMaster
	);
}
````
...
## Creating New Issues Automatically {#create-new-issues-automatically data-title="Create New Issues Automatically") 
