...
This feature is available in paid and trial ProGet editions. {.info} [[ATTN ALEX: Is this actually available in trials? I didn't think it was. If not, I'd revise the copy to something like: "This feature is available only in paid ProGet editions. Compare features by edition](/docs/proget/administration/license) or [request a quote](https://inedo.com/proget/pricing/request-quote)."]]
:::
...
Like an air traffic controller [[ATTN RICH: If there's a better analogy, this can be replaced. I just like the visual]], the load balancer will direct traffic bound for the ProGet web application, evenly distributing traffic during heavy load times.
...
**Session Affinity (sticky sessions) are not required.** While you can enable this, it's not required and may simplify your traffic and load balancer configuration. [[ATTN RICH: The pieces of the sentence don't seem to make sense together. I think it should either be "While sticky sessions are not a requirement, enabling them may simplify your traffic and load balancer configuration." OR "While you can enable this, it may simplify your traffic and load balancer configuration not to use them." I'm not sure which is correct.]]
...
The [Inedo Hub](/docs/proget/installation/installation-guide) installer included installing the ProGet service when installing ProGet on each Web Node. [[ATTN RICH - Once more, in English? haha]]
...
The service messenger is a component of ProGet that enables simple communications between web and service nodes. By default it uses a named pipe, but in a load-balanced configuration it needs to be configured to use TCP instead. The messenger is optional, but certain parts of ProGet's web interface may be slightly degraded without it. See the documentation on the [Service Messenger](/docs/proget/installation/installation-guide/service-messenger) for installation and configuration. [[ATTN RICH: Is this repeated from above? If it's not the same thing, please guide me on how to rewrite to differentiate. if it's the same, maybe we just lump it in with the previous section with a link back to that anchor?]]
