...
The `assets list` command set is used to list files and folders in an asset directory.
...
To check if a folder exists, you could either use the [Get Metadata](/docs/proget/reference-api/proget-api-assets-metadata-get) endpoint or inspect the array returned in `/endpoints/internal-files/dir/does/not` to see if the desired folder exists.
