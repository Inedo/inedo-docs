---
title: "Query Latest Packages"
order: 1
---

*Query Latest Package* is available as both a `pgutil` command and an HTTP Request, and will return a JSON array of [PackageVersionInfo](/docs/proget/reference-api/proget-api-packages#package-version) objects describing the latest version of a package in a feed that match the specified query arguments.

:::(Info) (🚀 Quick Example: Describing the latest package version with pgutil)
This example will describe the latest package version of  the package `myNugetPackage` in the feed `myNugetFeed`

```
pgutil packages latest --package=myNugetPackage --feed=myNugetFeed
```
:::

## Command Specification (CLI)
The `packages latest` command set is used to describe the latest version of the specified package in the feed. 

The `--package` option is always required, and the `--feed` option is required if there is no default feed configured. The `--stable` parameter is optional, but when set true, latest stable versions of packages are returned instead of absolute latest versions.

**Describing the latest NuGet package version** requires the package name (e.g. `myNugetPackage`)
```
pgutil packages latest --package=myNugetPackage --feed=myNugetFeed
```

**Describing the latest stable npm package version** requires the package name (e.g. `myNpmPackage`), the scope (e.g. `@myScope`) and setting `--stable` as `true`.
```
pgutil packages latest --package=@myScop/myNpmPackage --feed=myNpmFeed --stable=true
```

## HTTP Request Specification
To query a package version, simply `GET` to the URL with a feed name, `group name`, `package name` and `stableOnly` parameters, and an [appropriate API Key](/docs/proget/reference-api/proget-api-packages#authentication).
````
...
A successful (`200`) response body will contain an array of [PackageVersionInfo](/docs/proget/reference-api/proget-api-packages#package-version) objects. For example, to querying the latest stable version of `myNuGetPackage`, the request would return this:
```
