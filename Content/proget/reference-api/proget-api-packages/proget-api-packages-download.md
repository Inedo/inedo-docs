...
The `packages download` command set is used to list files and folders in an asset directory.

The `--package` and `--version` options are always required. The `--feed` option is required if there is no default feed configured, and the `--qualifier` option is required for multifile packages like Debian and RubyGems
...
```
pgutil packages download --feed=myNpmFeed --package=@scope/myNpmPackage --version=4.5.6 --output-file=C:\Temp\package.tgz
```
