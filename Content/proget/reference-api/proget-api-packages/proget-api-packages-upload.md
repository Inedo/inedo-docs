...
The `packages upload` command set is used to upload a package to a feed.
...
```
:::(internal)
**Uploading a Python Package** requires the name (e.g. `myPythonPackage`), version (e.g. `3.0.0`), a quoted qualifier with the filename (e.g. `filename=myPythonPackage.3.0.0.tgz`and input file path (e.g. `C:\inedo\packages\myPythonPackage.3.0.0.tgz`):

```
Run pgutil packages upload --feed=approved-pypi --input-file=C:\Django-5.0.6.tar.gz
```

**Uploading a Debian Package** requires the name (e.g. `myDebianPackage`), version (e.g. `4.0.0`), component (e.g. `main`), and a quoted qualifier with the architecture (e.g. `arch=amd64`):

```
pgutil packages upload --feed=approved-debian --input-file=C:\Temp\debhelper_13.15.3_all.deb
```
:::
