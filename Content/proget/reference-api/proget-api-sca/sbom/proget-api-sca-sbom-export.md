...
:::(Info) (🚀 Quick Example: Exporting SBOM with pgutil)
This example exports an SBOM of version `1.2.3` of a project `myProject` to the path `C:\inedo\sbom`
````
pgutil builds sbom --input=myProject --output=C:\inedo\sbom --project-name=myProject --version=1.2.3
````
:::

## Command Specification (CLI)
The `builds sbom` command set is used to generate and export a minimal SBOM document.
    
The `--input`, `--output`, `--project-name` and `--version` options are always required.
    
**Generating an SBOM of a project** requires the input (e.g. `MyNpmFeed`), output path (e.g. `C:\inedo\sbom`) project name (e.g. `name`) and version (e.g. `2.0.0`):
```
pgutil builds sbom --input=<input> --output=C:\inedo\sbom --project-name=<project-name> --version=2.0.0
```
