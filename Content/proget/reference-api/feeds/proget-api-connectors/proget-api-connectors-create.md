...
*Create Connector* is available as both a `pgutil` command and an HTTP Request, and will
 create a connector, with the properties defined by a [ProgetConnector](/docs/proget/reference-api/feeds/proget-api-connectors#connector-object) object in the body of the request.
...
The `connectors create` command set is used to create a connector.
...
To create a connector, simply `POST` to the URL with an [appropriate API Key](/docs/proget/reference-api/feeds/proget-api-connectors#authentication) and a [ProgetConnector](/docs/proget/reference-api/feeds/proget-api-connectors#connector-object) object as the request body.
