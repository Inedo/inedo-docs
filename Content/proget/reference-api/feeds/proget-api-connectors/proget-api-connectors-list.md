...
*List Connectors* is available as both a `pgutil` command and an HTTP Request, and will return an array of [ProGetConnector](/docs/proget/reference-api/feeds/proget-api-connectorsi#connector-object) objects, describing all existing connectors.
...
The `connectors list` command set is used to list all existing connectors.
...
To list all existing connectors, simply `GET` to the URL with an [appropriate API Key](/docs/proget/reference-api/feeds/proget-api-connectors#authentication).
