...
The Asset Directories API provides several `pgutil` commands (available as `pgutil assets`) and HTTP endpoints for querying, downloading, publishing, deleting, and performing other operations on files and folders in an [asset directory](/docs/proget/asset-directories-file-storage/what-is-an-asset-directory):
