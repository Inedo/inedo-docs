...
| Feed API Keys | `View/Download` is required for all endpoints<br/>`Add/Repackage` and `Overwrite/Delete` is required for [Import Universal Package](/docs/proget/api/universal-feed/import)<br/>`Add/Repackage` is required for [Create/Upload Universal Package](/docs/proget/api/universal-feed/upload) |
| Personal API Key | `Feeds_ViewFeed` is required for all endpoints<br/>`Feeds_AddPackage`, `Feeds_DeletePackage`, and `Feeds_DownloadPackage` is required for [Import Universal Package](/docs/proget/api/universal-feed/import)<br/>`Feeds_AddPackage` is required for [Create/Upload Universal Package](/docs/proget/api/universal-feed/upload) |
| No API Key | anonymous or authenticated user must have the same combination of above permissions |
