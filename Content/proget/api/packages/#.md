...
| --- | --- |
| System API Keys | At least `Use/Manage Feeds` is required for all endpoints. `Full Control` also grants permissions for all endpoints.   
...
| Personal API Key | `View & Download Packages` is required for [List Packages](/docs/proget/api/packages/list) and [List Package Versions](/docs/proget/api/packages/list-versions)<br/>`Publish Packages` is required for [Upload Package](/docs/proget/api/packages/upload)<br/>`Promote Packages` is required for [Promote Package](/docs/proget/api/packages/promote)<br/>`Administer` or `Manage Feed` is required for [Delete Package](/docs/proget/api/packages/delete), [Set Package Status](/docs/proget/api/packages/status), and [Repackage Package](/docs/proget/api/packages/repackage). These will also grant permissions for all other endpoints.
