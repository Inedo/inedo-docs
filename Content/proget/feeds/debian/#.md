...
## Authenticating to Debian Feeds

If you've [configured your feed to require authentication](/docs/proget/administration-security/proget-howto-configure-permissions-and-restrictions-on-feeds), you must add your credentials when adding your Debian feed as a source. While you can authenticate with a username and password, we strongly recommend [creating and using an API Key](/docs/proget/reference-api/proget-apikeys) for authentication, using `api` as the username and your API key as the password. To add a feed with authentication to your client, run:

```bash
$ echo "deb http://api:«api-key»@«proget-server»/debian/«feed-name» «distro-name» «component-name»" | sudo tee /etc/apt/sources.list.d/«feed-name».list
```

## Installing and Creating Debian Packages 
...
To learn how to create a Debian package, read [Creating a Debian package](https://debian-handbook.info/browse/stable/debian-packaging.html) chapter of the Debian Administrator's handbook.
...
To publish a Debian package to a ProGet feed, you can use [pgutil](/docs/proget/reference-api/proget-pgutil). This will require some [minor configuration](/docs/proget/reference-api/proget-pgutil#sources) before use. This includes setting up your ProGet instance and API key as a source by running:
...
```bash
$ pgutil packages upload --feed=«feed-name» --input-file=«path-to-package» --distribution=«distribution»
```
...
```sh
curl http://«proget-server»/debian/«feed-name»/upload/«distro-name»/«component-name» --user «username»:«password» --upload-file my-package.deb
```

#### wget:

```sh
wget http://«proget-server»/debian/«feed-name»/upload/«distro-name»/«component-name» --http-user «username» --http-password «password» --method POST --body-file my-package.deb
```

#### PowerShell:

```powershell
Invoke-WebRequest http://«proget-server»/debian/«feed-name»/upload/{distro-name}/{component-name} -Credential [System.Net.NetworkCredential]::new('«username»', '«password»') -Method PUT -InFile my-package.deb
```
...
* Source packages can often be useful when using the Debian packaging tools (apt, apt-get, aptitude) to download the source package, especially for debugging purposes. Without the source package, one can only get the stack trace, but not really inspect the code.

* When upgrading from one Debian release (Ubuntu 19.10) to the next version (Ubuntu 20.04), the recommended method is to download all source packages from the previous repository and then rebuild them in the new operating system. This is the standard best practice for seeding a new apt repository
