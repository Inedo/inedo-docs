...
Note that this feed type for VS Code style extensions only, **not** for Visual Studio extensions. See [Visual Studio Extension feeds](vsix) if you need a feed for Visual Studio extensions.

## Prerequisite Configuration

## Using With Visual Studio Code

The official Microsoft-produced build of Visual Studio code is not currently supported by this feed type. While it is technically possible to configure it to install extensions from an Open VSX feed, this requires hand-editing a file that is part of its distribution and gets overwritten on upgrades. Further, changing the to a non-Microsoft private extension repository in this way may violate Microsoft's license.

### Adding the Feed to VSCodium

These instructions are specifically for VSCodium, but should also apply to other open source fork of Visual Studio Code.
