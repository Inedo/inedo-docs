...
With ProGet, you can use NuGet packages to distribute private proprietary libraries within your organization. To do this, simply create a NuGet feed: "Feeds" > "Create New Feed" and select "NuGet (.NET Packages").

Then configure [Visual Studio](/docs/proget/feeds/nuget/howto-nuget-visualstudio) or [NuGet CLI](/docs/proget/feeds/nuget/howto-nuget-proxy#add-feed) to use your feed as a new package source.
...
You can read more about creating and publishing packages in our [HOWTO: Publish NuGet Packages to a Private Repository in ProGet](/docs/proget/feeds/conda/howto-nuget-publish) guide.
...
## Authenticating to a NuGet Feed
...
```bash
$ nuget push MyPackage -Source http://proget.corp.local/nuget/internal-nuget/v3/index.json -ApiKey john:abc12345
```
...
To add and authenticate the feed in NuGet CLI, enter the following:

```bash
$ nuget setApiKey «api-key» -Source «feed-url»
```
