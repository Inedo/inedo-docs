...
To unlist a package you can use the `pgutil packages status unlisted` command in [pgutil](/docs/proget/reference-api/proget-pgutil). For example, if unlisting version `1.2.3` of the package `MyPackage` from the feed `internal-nuget` you would enter:
...
To delete a package you can use the `pgutil packages delete` command in [pgutil](/docs/proget/reference-api/proget-pgutil). For example, if deleting version `1.2.3` of the package `MyPackage` from the feed `internal-nuget` you would enter:
