...
To unlist a package you can use the [`packages status unlisted`](/docs/proget/api/packages/proget-api-packages-status) command in [pgutil](/docs/proget/api/pgutil). For example, if unlisting version `1.2.3` of the package `MyPackage` from the feed `internal-nuget` you would enter:
