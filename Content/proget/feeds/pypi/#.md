...
To yank a package in ProGet, you can do so either in the ProGet UI or using [pgutil](/docs/proget/api/pgutil) with the [`set package status`](/docs/proget/api/packages/proget-api-packages-status) command: 
