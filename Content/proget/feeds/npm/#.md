...
This page covers setting up [npm](https://www.npmjs.com/) and [Yarn](https://yarnpkg.com/) clients with ProGet. Additionally, for those interested in using [pnpm](https://pnpm.io/), we’ll also touch on how to configure this with ProGet.

## Adding an npm Feed as a Source

To publish and install packages from your npm feed in ProGet, you'll need to add it as a source in your npm client. To do this you will need to use the `npm config set registry` or `yarn config set registry` commands: 
...
You can confirm that the feed has been set correctly by using `npm get registry` for npm, `yarn config get registry` for Yarn. To add a source when using pnpm, you will need to configure this in your [.npmrc file](https://docs.npmjs.com/cli/v10/configuring-npm/npmrc).
