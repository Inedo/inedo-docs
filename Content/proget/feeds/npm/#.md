...
## (Optional) Authenticating to npm Feeds
...
Next you'll need to add an `_auth` token. This is a username and password string `«username»:«password»` that's been base64-encoded We strongly recommend using your API key for this, with api as the username, and then API Key as the password. 
...
To encode your API key in base64 you can use this PowerShell script:
...
Then, if you haven't already, configure your local environment with your npm feed by entering: 
...
```bash
$ npm config set //«proget-url»/npm/«feed-name»/:_auth «encoded-auth-token»
```
