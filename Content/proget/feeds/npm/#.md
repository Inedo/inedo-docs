...
To create an npm feed and configure it with your npm client you can read our [HOWTO: Proxy Packages from the npm Registry in ProGet](/docs/proget/feeds/npm/howto-npm-proxy) guide.
...
To authenticate you'll need to add an `_auth` token. This is a username and password string `«username»:«password»` that's been base64-encoded. 
...
```
Or press F12 to open the Developer Tools, click on the "Console", then run the following script (replace `«username»:«password»`).
