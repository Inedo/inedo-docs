...
BuildMaster provides two options for deploying Docker containers; Docker Run and Docker Compose.
...
#### Docker Compose
BuildMaster's [Docker Compose Files](/docs/buildmaster/docker/compose-file) feature lets you configure more advanced Docker configurations.  Similiar to Docker Run Configs, Docker Compose deployments let you:
* Maintain different files across testing and production environments on one editor
* Tracks a compose file revision to a release
* Track who changed which configuration values over time
* Restrict which users can view or edit configuration based on environment

![BuildMaster-DockerRunConfig](/resources/docs/buildmaster-edit-docker-compose-file.png){height="" width=""}
...
In general, we recommend having one Docker Repository per application in BuildMaster, and one image per build, but you can connect as many repositories to your application as needed, and build as many images as you'd like. However, with multiple images per build, you won't be able to rely on values of `$DockerRepository`, `$DockerRepositoryResource`, and `$DockerTag` in your build and deployment scripts.
:::
...
There are multiple Docker Repository Resource types that you can choose from when connecting to a Docker repository. These different types make it easier to work with different repositories in your scripts; you can use simpler names and not worry about server/host addresses or authentication. To log in to your Docker registry in OtterScript, use the `Docker::Login` operation and `Docker:Logout` operation to log out.
...
Docker Repositories are associated with builds in BuildMaster and can be referenced in OtterScript using the `$DockerRepository` build variable for the repository URL or use the `$DockerRepositoryResource` build variable to reference the [repository resource](#docker-repository-resources).  The Docker Repository will be set when an image is built using `Docker::Build-Image` or tagged using `Docker::Tag`, an image and tag will then be attached with your build.  After an image has been attached to your build, the tag of the most recently attached image will be stored in the `$DockerTag` build variable.

::: (Info) (Note:)
If you are using a Docker Repository stored in ProGet, the attached image will be linked directly to your image in ProGet.
:::
