...
The [Create-File Operation](/docs/buildmaster-reference-operations-files-create-file) creates the file you specify. If the file already exists, it will delete the old file and create a new one. Or, it will throw an error if  `Overwrite` is not set to `true`. 
...
```
The [Ensure-File Operation](/docs/buildmaster-reference-operations-files-ensure-file) will also create the file that you specify. But if the file exists, it will update only the properties you specify.
