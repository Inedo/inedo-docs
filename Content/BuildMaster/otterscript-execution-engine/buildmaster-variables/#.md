...
| **Variable functions** | predefined functions like `$Date()` or `$ServerName()` that will automatically change depending on when and where you scripts are run; although the parentheses are optional, some variable functions take parameters, like `$FileExists(fileName)`.<br /><br/> You can create your own variable functions with the [Inedo SDK](/inedosdk-versions-release-notes). |
