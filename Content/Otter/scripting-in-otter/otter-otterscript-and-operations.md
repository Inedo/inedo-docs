...
The [Create-File Operation](/docs/otter-reference-operations-files-create-file) can be really simple: it just creates the file you specify and will raise throw an error if the file exists (and you don't specify `Overwrite` to be `true`).
...
On the other hand, even though the [Ensure-File Operation](/docs/otter-reference-operations-files-ensure-file) looks nearly the same, it's a bit trickier behind the scenes:
